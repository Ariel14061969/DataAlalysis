...
def single_continent_plots(df_continent,continent_name, log_file):
    ...
    #sns.barplot(x=df_continent.index, y='gdp', data=df_continent, color='skyblue', ax=ax1, label='GDP')
    sns.barplot(x=df_continent.index, y='gdp', data=df_continent, color='green', ax=ax1, label='GDP')
    ax1.set_xlabel('Country')
    ax1.set_ylabel('GDP [Millions of $]', color='green')
    ax1.tick_params(axis='y', labelcolor='green')
    ax1.set_xticks(range(len(df_continent.index)))  # Explicitly set tick locations
    ax1.set_xticklabels(df_continent.index, rotation=90, ha='right')

    # Create a second y-axis that shares the same x-axis
    ax2 = ax1.twinx()

    # Plot GDP per Capita on the second y-axis
    sns.lineplot(x=df_continent.index, y='gdp_per_capita', data=df_continent, color='royalblue', marker='o', ax=ax2,
